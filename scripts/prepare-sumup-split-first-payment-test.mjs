...
if (process.env.VERCEL_ENV !== 'preview') {
  console.log('[split-sandbox-1] omitido fuera de Preview');
  process.exit(0);
}
...
if (!supabaseUrl || !serviceKey || !apiKey || !merchantCode || !publicBaseUrl || bypassSecret.length !== 32) {
  console.error('[split-sandbox-1] configuración incompleta');
  process.exit(1);
}
...
const existing = await supabase
  ...
  .eq('last_name', 'Split Pago')
  .like('email_normalized', 'sandbox-split-%@example.invalid')
  .limit(1)
  .maybeSingle();

if (existing.error) throw existing.error;
if (existing.data) {
  console.log(`[split-sandbox-1] test ya preparado: ${existing.data.order_reference}`);
  process.exit(0);
}

const suffix = Math.random().toString(36).slice(2, 10).toUpperCase().padEnd(8, '0').slice(0, 8);
const orderReference = `GHC-${suffix}`;
const requestKey = `sandbox-split-${Date.now()}-${suffix}`;
...
const draft = await rpc('preventa_create_draft_v1', {
  ...
  p_last_name: 'Split Pago',
  p_email: `sandbox-split-${suffix.toLowerCase()}@example.invalid`,
  p_country: 'España',
  p_phone: '',
  p_payment_plan: 'split',
  p_total_amount_cents: 179000,
  p_first_installment_cents: 89500,
  p_second_installment_cents: 89500,
  ...
  p_source_channel: 'sandbox_split_payment_test',
  p_source_detail: 'manual_hosted_checkout_split_first',
  p_campaign_code: '',
  p_closer_code: '',
});
...
const createResponse = await fetch('https://api.sumup.com/v0.1/checkouts', {
  ...
  body: JSON.stringify({
    ...
    currency: 'EUR',
    description: 'GHC Academy · Edición Fundadora · PRIMERA CUOTA · PRUEBA SANDBOX',
...
console.log(`[split-sandbox-1] orderReference=${orderReference}`);
console.log(`[split-sandbox-1] checkoutReference=${checkoutReference}`);
console.log(`[split-sandbox-1] checkoutId=${checkout.id}`);
console.log(`[split-sandbox-1] hostedCheckoutUrl=${checkout.hosted_checkout_url}`);
console.log(`[split-sandbox-1] heldUntil=${heldUntil}`);
console.log('[split-sandbox-1] listo: primera cuota 895 EUR ficticios; matrícula split retenida para webhook real');
