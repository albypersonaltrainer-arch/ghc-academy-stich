...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('installment_no',i.installment_no,'amount_cents',i.amount_cents,'due_at',i.due_at) order by i.installment_no),'[]'::jsonb)
    into v_installments from public.academy_installments i where i.order_id=v_order.id;
