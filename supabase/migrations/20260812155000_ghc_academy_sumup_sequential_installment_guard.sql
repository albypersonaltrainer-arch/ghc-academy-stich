...
begin
  ...
  if p_installment_no < 1 or p_installment_no > 4 then raise exception 'Número de cuota no válido.'; end if;
  ...
  if v_installment.id is null then raise exception 'Cuota Academy no encontrada.'; end if;
  if v_installment.status = 'paid' then raise exception 'Esta cuota ya está pagada.'; end if;
  ...
  if exists (
    select 1
    from public.academy_installments prior
    where prior.order_id = v_order.id
      and prior.installment_no < v_installment.installment_no
      and prior.status <> 'paid'
  ) then
    raise exception 'Debes completar primero la cuota anterior.';
  end if;
...
begin
  ...
  if coalesce(trim(p_idempotency_key),'') = '' then raise exception 'Falta clave de idempotencia.'; end if;

  select * into v_attempt from public.academy_payment_attempts where idempotency_key = p_idempotency_key;
  if v_attempt.id is not null then
    return jsonb_build_object(
      'attempt_id',v_attempt.id,'order_id',v_attempt.order_id,'installment_id',v_attempt.installment_id,
      'provider_checkout_id',v_attempt.provider_checkout_id,'status',v_attempt.status,'idempotent_replay',true
    );
  end if;
  ...
  if v_order.id is null or v_installment.id is null then raise exception 'Pedido o cuota Academy no encontrados.'; end if;
  if v_order.status in ('cancelled','refunded','chargeback','suspended') then raise exception 'El pedido no admite checkout.'; end if;
  if v_installment.status in ('paid','cancelled','refunded','chargeback') then raise exception 'La cuota no admite checkout.'; end if;
  ...
  if exists (
    ...
    raise exception 'La cuota anterior debe estar pagada antes de crear este checkout.';
  end if;

  if v_installment.amount_cents <> p_expected_amount_cents then raise exception 'Importe de checkout distinto del importe de la cuota.'; end if;
  if v_order.currency <> upper(trim(p_currency)) then raise exception 'Moneda de checkout distinta de la moneda del pedido.'; end if;

  insert into public.academy_payment_attempts(
    order_id,installment_id,provider,checkout_reference,provider_checkout_id,hosted_checkout_url,
    expected_amount_cents,currency,status,idempotency_key,provider_metadata
  ) values (
    v_order.id,v_installment.id,'sumup',trim(p_checkout_reference),trim(p_provider_checkout_id),
    nullif(trim(p_hosted_checkout_url),''),p_expected_amount_cents,v_order.currency,'pending',trim(p_idempotency_key),
    coalesce(p_provider_metadata,'{}'::jsonb)
  ) returning * into v_attempt;

  update public.academy_installments
  set provider='sumup',provider_checkout_id=v_attempt.provider_checkout_id,
      metadata=metadata||jsonb_build_object('sumup_checkout_reference',v_attempt.checkout_reference,'sumup_attempt_id',v_attempt.id),
      updated_at=now()
  where id=v_installment.id;

  update public.academy_orders
  set provider='sumup',provider_reference=coalesce(provider_reference,v_attempt.provider_checkout_id),updated_at=now()
  where id=v_order.id;

  insert into public.academy_commercial_events(order_id,event_type,actor_type,message,payload)
  values(v_order.id,'sumup_checkout_created','provider','Checkout SumUp creado para cuota Academy',
    jsonb_build_object('attempt_id',v_attempt.id,'installment_id',v_installment.id,'installment_no',v_installment.installment_no,
      'provider_checkout_id',v_attempt.provider_checkout_id,'checkout_reference',v_attempt.checkout_reference,
      'expected_amount_cents',v_attempt.expected_amount_cents));

  return jsonb_build_object('attempt_id',v_attempt.id,'order_id',v_order.id,'installment_id',v_installment.id,
    'provider_checkout_id',v_attempt.provider_checkout_id,'status',v_attempt.status,'idempotent_replay',false);
end;
$$;
...
begin
  ...
  if exists (
    ...
    raise exception 'No se puede confirmar esta cuota mientras exista una cuota anterior sin pagar.';
  end if;

  update public.academy_payment_attempts
  set status='paid',provider_payment_id=trim(p_provider_payment_id),terminal_at=coalesce(p_occurred_at,now()),
      provider_metadata=provider_metadata||coalesce(p_provider_metadata,'{}'::jsonb),updated_at=now()
  where id=v_attempt.id;

  update public.academy_installments
  set status='paid',provider='sumup',provider_checkout_id=v_attempt.provider_checkout_id,
      provider_payment_id=trim(p_provider_payment_id),paid_amount_cents=p_amount_cents,
      paid_at=coalesce(p_occurred_at,now()),failed_at=null,
      metadata=metadata||jsonb_build_object('sumup_attempt_id',v_attempt.id,'sumup_checkout_reference',v_attempt.checkout_reference)
        ||coalesce(p_provider_metadata,'{}'::jsonb),updated_at=now()
  where id=v_installment.id;
  ...
  insert into public.academy_commercial_events(order_id,event_type,actor_type,message,payload)
  values(v_order.id,'installment_paid_sumup','provider','Cuota Academy confirmada por SumUp',
    jsonb_build_object('attempt_id',v_attempt.id,'installment_id',v_installment.id,'installment_no',v_installment.installment_no,
      'provider_checkout_id',v_attempt.provider_checkout_id,'provider_payment_id',trim(p_provider_payment_id),'amount_cents',p_amount_cents));

  v_state:=private.ghc_apply_academy_order_state(v_order.id,'provider');
  return v_state||jsonb_build_object('attempt_id',v_attempt.id,'installment_id',v_installment.id,
    'installment_no',v_installment.installment_no,'provider_payment_id',trim(p_provider_payment_id),'idempotent_replay',false);
end;
$$;
