...
begin
  ...
  if (select count(*) from public.preventa_payments p join public.preventa_orders o on o.id=p.order_id where o.order_reference='GHC-TEST0001') <> 1 then
    raise exception 'FAIL: pago único debe tener una cuota';
  end if;
end $$;
...
-- mismo provider_payment_id, clave distinta: debe ser replay sin efectos nuevos
select public.preventa_confirm_payment_v1(
...
begin
  ...
  if (select commission_base_cents from public.preventa_attribution where order_id=v.id) <> 0 then
    raise exception 'FAIL: refund no revierte comisión';
  end if;
end $$;
...
begin
  ...
  if (select due_at from public.preventa_payments where order_id=v.id and installment_no=2) <> expected_due then
    raise exception 'FAIL: due_at cuota 2';
  end if;
...
begin
  ...
  if (select status from public.preventa_checkout_attempts where provider_checkout_id='sumup-checkout-split-002') <> 'paid' then
    raise exception 'FAIL: intento segunda cuota no quedó paid';
  end if;
end $$;
...
begin
  ...
  if not has_function_privilege('service_role','public.preventa_confirm_payment_v1(text,smallint,integer,text,text,timestamptz,jsonb)','EXECUTE') then
    raise exception 'FAIL SECURITY: service_role sin execute';
  end if;
end $$;
