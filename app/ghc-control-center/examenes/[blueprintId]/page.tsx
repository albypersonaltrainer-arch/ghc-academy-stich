...
const BUILD_ID = "GHC-EXAM-MANUAL-BRIDGE-V4 · importación controlada · sin duplicados";
const GREEN = "#63E546";
...
export default function BlueprintDetailPage({ params }: PageProps) {
  ...
  function openImporter() {
    ...
    setMessage("Importador abierto. Pega el JSON, valida primero y después pulsa Importar definitivamente.");
    setValidationMessage("");
    setLastImportSummary("");

    window.setTimeout(() => {
      const section = document.getElementById("importar");
      section?.scrollIntoView({ behavior: "smooth", block: "start" });
      const textarea = document.getElementById("json-import-textarea") as HTMLTextAreaElement | null;
      textarea?.focus();
    }, 80);
  }
  ...
  function validateJsonOnly() {
    if (!blueprint) {
      setValidationMessage("No hay blueprint cargado.");
    ...
    try {
      ...
      const expected = Number(blueprint.requested_question_count || 0);
      const answerCount = Number(blueprint.answer_count || 4);
      const warning = expected && normalizedQuestions.length !== expected
        ? ` Aviso: el blueprint pedía ${expected} preguntas y el JSON trae ${normalizedQuestions.length}.`
        : "";

      setValidationMessage(`JSON válido: ${normalizedQuestions.length} preguntas detectadas · ${normalizedQuestions.length * answerCount} opciones previstas.${warning}`);
      setMessage("JSON validado correctamente. Si todo está bien, pulsa Importar definitivamente.");
    } catch (error) {
      const msg = getErrorMessage(error);
      setValidationMessage(`JSON no válido: ${msg}`);
      setMessage(msg);
    }
  }
  ...
  async function importJson() {
    ...
    setBusy(true);
    setMessage("Validando JSON de preguntas antes de importar...");
    setLastImportSummary("");
    ...
    try {
      ...
      if (questions.length > 0) {
        const confirmed = window.confirm(`Este borrador ya tiene ${questions.length} preguntas importadas. Importar de nuevo puede añadir más preguntas. ¿Quieres continuar igualmente?`);
        if (!confirmed) {
          setMessage("Importación cancelada. No se ha modificado Supabase.");
          setBusy(false);
          return;
        }
      }

      setValidationMessage(`JSON válido: ${normalizedQuestions.length} preguntas detectadas. Importación en curso...`);
      ...
      setShowImporter(true);
      await loadBlueprint();
      setLastImportSummary(`Importación completada: ${normalizedQuestions.length} preguntas guardadas como borrador para revisión humana. Opciones previstas: ${normalizedQuestions.length * Number(blueprint.answer_count || 4)}.`);
      setValidationMessage("");
      setMessage(`Importación completada: ${normalizedQuestions.length} preguntas guardadas. Revisa el bloque Preguntas importadas.`);
      window.setTimeout(() => {
    ...
    <main className="page">
      ...
      <section className="shell">
        ...
        <section className="hero">
          ...
          <div className="hero-actions"><button type="button" onClick={copyPrompt}>Copiar prompt para ChatGPT/Claude</button><button type="button" onClick={openImporter}>Importar JSON de preguntas</button></div>
        </section>
        ...
        <section className="main-grid">
          <div className="left-column">
            ...
            <article className="card" id="importar"><div className="card-head"><div><p className="kicker">Importación</p><h2>Pegar JSON de preguntas</h2></div><button type="button" onClick={() => showImporter ? setShowImporter(false) : openImporter()}>{showImporter ? "Cerrar importador" : "Abrir importador"}</button></div><p className="muted">Pega aquí el JSON devuelto por ChatGPT/Claude. Primero valida la estructura y después importa definitivamente. GHC Academy guardará las preguntas como draft_ai, nunca publicadas.</p>{questions.length ? <div className="import-warning"><strong>Este borrador ya tiene {questions.length} preguntas importadas.</strong><p>Si importas de nuevo, el sistema pedirá confirmación para evitar duplicados.</p></div> : null}{lastImportSummary ? <div className="import-success"><strong>Importación completada</strong><p>{lastImportSummary}</p></div> : null}{showImporter ? <div className="json-importer"><textarea id="json-import-textarea" value={jsonInput} onChange={(event) => { setJsonInput(event.target.value); setValidationMessage(""); setLastImportSummary(""); }} placeholder='{"questions":[{"question":"...","question_type":"test","options":[{"label":"A","text":"...","is_correct":false}],"correct_label":"A","explanation":"...","difficulty":"medium","evaluated_objective":"..."}]}' />{validationMessage ? <div className={validationMessage.startsWith("JSON válido") ? "import-success" : "import-error"}><strong>{validationMessage.startsWith("JSON válido") ? "Validación correcta" : "Validación fallida"}</strong><p>{validationMessage}</p></div> : null}<div className="import-actions"><button type="button" disabled={busy || !jsonInput.trim()} onClick={validateJsonOnly}>Validar JSON</button><button type="button" disabled={busy || !jsonInput.trim()} onClick={importJson}>{busy ? "Importando..." : "Importar definitivamente"}</button><button type="button" disabled={busy} onClick={() => { setJsonInput(exampleJson(Number(blueprint.answer_count || 4))); setValidationMessage(""); setLastImportSummary(""); }}>Insertar ejemplo</button></div></div> : null}</article>
          </div>
