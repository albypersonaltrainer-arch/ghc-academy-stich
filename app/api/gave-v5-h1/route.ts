...
const SCENES: Record<string, string> = {
  ...
  '4': 'El entrenamiento personal no consiste en elegir ejercicios al azar. Empieza entendiendo a la persona: qué quiere conseguir, qué puede hacer hoy y qué factores pueden limitar su progreso. Después se evalúa lo mínimo útil, se seleccionan patrones de movimiento, se ajusta la carga y se observa la respuesta. Una técnica correcta no es una postura congelada: es una ejecución segura, eficiente y adecuada para esa persona. El entrenador decide, aplica, observa y corrige. Eso es lo que diferencia acompañar una sesión de construir un proceso de entrenamiento. En el sistema G, H, C, cada sesión debe dejar información para tomar una mejor decisión en la siguiente.',
};
