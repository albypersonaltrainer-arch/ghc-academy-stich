  ...
  useEffect(() => {
    async function loadCourse() {
      try {
        ...
        const courseResponse = await fetch(courseEndpoint, {
          headers: {
            apikey: supabaseKey,
    ...
    return (
      <main style={pageStyle}>
        <div style={containerStyle}>
          <p style={{ color: neon, fontWeight: 900 }}>Cargando curso...</p>
        </div>
      </main>
    );
  }
    ...
    return (
      <main style={pageStyle}>
        <div style={containerStyle}>
          ...
          <p style={textStyle}>Este contenido no existe o todavía no está publicado.</p>
        </div>
      </main>
    );
  }
    ...
    <main style={pageStyle}>
      <div style={containerStyle}>
        ...
        <section style={heroStyle}>
          <div>
            <p style={eyebrowStyle}>GHC Academy · Formación premium</p>
            ...
            <p style={textStyle}>
              {course.description || 'Contenido académico premium basado en ciencia real.'}
            </p>
          </div>
        ...
        <section style={{ marginTop: '34px' }}>
          <p style={sectionLabel}>Sistema académico</p>
          <h2 style={sectionTitle}>Módulos del curso</h2>

          {modules.length === 0 && (
            <div style={emptyBox}>
              Todavía no hay módulos creados para este curso.
            </div>
          )}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '18px' }}>
                    <div>
                      ...
                      <p style={textStyle}>
                        {module.description ||
                          'Módulo formativo estructurado para avanzar de forma progresiva.'}
                      </p>
                    </div>
                  ...
                  <div style={{ marginTop: '20px', display: 'grid', gap: '10px' }}>
                    {moduleLessons.length === 0 && (
                      <div style={lessonRow}>Lecciones pendientes de crear</div>
                    )}

                    {moduleLessons.map((lesson) => (
                      <div key={lesson.id} style={lessonRow}>
                        <span>{lesson.title}</span>
                        <span>{isUnlocked ? 'Abrir' : '🔒'}</span>
                      </div>
                    ))}
                  </div>
                </article>
